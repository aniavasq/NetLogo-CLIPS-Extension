...
public class Environment {
  ...
  static {
    ...
    switch (OsUtils.getOsName()) {
      case MacOS:
        libName = "libCLIPSJNI.jnilib";
        break;
      case Windows:
        libName = "CLIPSJNI.dll";
        break;
      case Linux:
        libName = "libCLIPSJNI.so";
        break;
      default:
        libName = "libCLIPSJNI.so";
        break;
  ...
  private static String convertStreamToString(java.io.InputStream is) {
    ...
    java.util.Scanner tmp = new java.util.Scanner(is, "UTF-8");
    java.util.Scanner s = tmp.useDelimiter("\\A");
  ...
  /******************************************/
  private native int addUserFunction(long env, String functionName, String returnTypes, int minArgs, int maxArgs,
      String restrictions, UserFunction callback);
  ...
  public void addUserFunction(String functionName, String returnTypes, int minArgs, int maxArgs, String restrictions,
    ...
    rv = addUserFunction(theEnvironment, functionName, returnTypes, minArgs, maxArgs, restrictions, callback);
  ...
  /* addRouter: */
  /**************/
  public void addRouter(Router theRouter) {
    if (theRouter == null) {
  ...
  public void checkForErrors(String function) throws CLIPSLoadException {
    ...
    for (CLIPSLineError theError : errorList) {
      exceptionString = exceptionString.concat(
          "\n" + theError.getFileName() + " (Line " + theError.getLineNumber() + ") : " + theError.getMessage());
    }
  ...
  /* getFactSlot: */
  /****************/
  public static PrimitiveValue getFactSlot(FactAddressValue theFact, String slotName) {
  ...
  /* directGetSlot: */
  /******************/
  public static PrimitiveValue directGetSlot(InstanceAddressValue theInstance, String slotName) {
    PrimitiveValue theValue;
  ...
  public void retainFact(FactAddressValue theFact) {
    retainFact(theFact.getEnvironment(), theFact.getEnvironment().getEnvironmentAddress(), theFact.getFactAddress());
  }
  ...
  public void releaseFact(FactAddressValue theFact) {
    releaseFact(theFact.getEnvironment(), theFact.getEnvironment().getEnvironmentAddress(), theFact.getFactAddress());
  }
  ...
  /* destroy: */
  /************/
  public void destroy(boolean throwError) {
    for (String key : routerMap.keySet()) {
      try {
        deleteRouter(routerMap.get(key));
      } catch (Exception e) {
